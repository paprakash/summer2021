...
double the_gausppar(double* vars, double* pars){
    ...
    pars[3]*TMath::Gaus(vars[0],pars[1],pars[2]);}
...
int macro9(){
    ...
    double a=15; double b=90; double c=7;
    double norm=0.6; double mean=125; double sigma=1.6;
    gausppar.SetParameters(norm,mean,sigma,a,b,c);
    gausppar.SetParNames("Norm","Mean","Sigma","a","b","c");
    format_line(&gausppar,kBlue,1);

    TH1F histo("histo","Signal plus background;X vals;Y Vals",50,80,160);
    ...
    gausppar.SetParameter(1,120);
    gausppar.SetParameter(2,2);
    gausppar.SetParameter(3,17);
    gausppar.SetParameter(4,91);
    gausppar.SetParameter(5,9);
