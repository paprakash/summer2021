...
double the_gausppar(double* vars, double* pars){
    ...
    pars[6]*TMath::Exp(pars[7]*(vars[0]-pars[8]));}


//Defining the fitting function
double the_fitter(double* vars, double* pars){
    return pars[0]*TMath::Gaus(vars[0],pars[1],pars[2])+
    pars[3]*TMath::BreitWigner(vars[0],pars[4],pars[5]);}
...
int macro11(){
    ...
    TF1 exp("exp","[0]*TMath::Exp([1]*(x-[2]))",80,160);
    format_line(&exp, kYellow,2);

    TF1 gausppar("gausppar",the_gausppar,80,160,9);
    double a=15; double b=90; double c=35;
    double norm=0.06; double mean=125; double sigma=1.6;
    double d=20; double e=-0.06; double f=0;

    gausppar.SetParameters(norm,mean,sigma,a,b,c,d,e,f);
    gausppar.SetParNames("Norm","Mean","Sigma","a","b","c","d","e","f");
    ...
    TF1 fitter("fitter",the_fitter,80,160,6);
    format_line(&fitter,kCyan,1);

    // Reset the parameters before the fit and set
    // by eye a peak at 6 with an area of more or less 50
   fitter.SetParameter(0,1);
   fitter.SetParLimits(1,120,130);
   fitter.SetParLimits(2,0,10);
   fitter.SetParameter(3,17);
   fitter.SetParameter(4,91);
   fitter.SetParLimits(5,20,55);
//   gausppar.setparameter(6,20);
//   gausppar.setparlimits(7,-0.3,-0.0001);
//   gausppar.setparlimits(8,-10,10);
...
//  for (int ipar=2;ipar<npar;++ipar) gausppar.SetParameter(ipar,1);
   ...
   // perform fit ...
   auto fitResPtr = histo.Fit(&fitter, "S");
   ...
   for (int ipar=0;ipar<3;ipar++){
       gaussian.SetParameter(ipar,fitter.GetParameter(ipar));
       britw.SetParameter(ipar,fitter.GetParameter(ipar+3));
//       exp.SetParameter(ipar,gausppar.GetParameter(ipar+6));
   }
